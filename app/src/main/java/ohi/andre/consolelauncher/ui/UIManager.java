...
import ohi.andre.consolelauncher.features.settings.options.Toolbar;
import ohi.andre.consolelauncher.features.settings.options.Ui;
import ohi.andre.consolelauncher.ui.suggestions.SuggestionTextWatcher;
import ohi.andre.consolelauncher.ui.suggestions.SuggestionsManager;
...
import ohi.andre.consolelauncher.ui.device.info.DeviceInfoManager2;
    ...
    public UIManager (Context context, final ViewGroup rootView, MainPack mainPack, boolean canApplyTheme, CommandExecuter executer) {
            ...
            public void onReceive (Context context, Intent intent) {
                ...
                if (action.equals(ACTION_UPDATE_SUGGESTIONS)) {
                    ...
                    try {
                        ...
                        fos.write(terminalManager.getTerminalText()
                                .getBytes());
        ...
        disposableSet.add(
                settingsManager.requestUpdates(Behavior.auto_scroll, Boolean.class)
                        ...
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(autoScroll -> rootView.getViewTreeObserver()
                                .addOnGlobalLayoutListener(() ->
                                {
                                    int heightDiff = rootView.getRootView()
        ...
        disposableSet.add(
                initViewsSingle.subscribe(list -> {
                    ...
                    ((View) terminalView.getParent()
                            .getParent()).setOnTouchListener(UIManager.this);
                })
        );
        ...
        disposableSet.add(
                Observable.combineLatest(initViewsSingle.toObservable(),
                        ...
                        .observeOn(AndroidSchedulers.mainThread())
                        .map(showSubmit -> showSubmit ? View.VISIBLE : View.GONE)
                        .subscribe(submitView::setVisibility)
        );
                ...
                settingsManager.requestUpdates(Suggestions.show_suggestions, Boolean.class)
                        .subscribe(show -> {
                            if (show) {
                                ...
                                rootView.findViewById(R.id.suggestions_group)
                                        .setVisibility(View.GONE);
                            }
                        })
        );
        
        deviceInfoTextView = rootView.findViewById(R.id.device_info_textview);
        
        // device info
        disposableSet.add(
                new DeviceInfoManager2(context)
                        .deviceInfoObservable()
                .subscribe(deviceInfoTextView::setText)
        );
    ...
    private void registerLockReceiver() {
        ...
        lockReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (strAction != null &&
                        (strAction.equals(Intent.ACTION_USER_PRESENT) || strAction.equals(Intent.ACTION_SCREEN_OFF) || strAction.equals(Intent.ACTION_SCREEN_ON)))
